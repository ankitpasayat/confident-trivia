 ...
 * 
 * Usage:
 *   npm run analyze-questions              (10 batches, no question details)
...
async function analyzeDistribution(
      ...
      if (showQuestions) {
        ...
        questions.forEach((q, idx) => {
          ...
          if (q.type === 'multiple-choice' && q.options) {
            ...
            console.log(`   ✓ Answer: ${String.fromCharCode(65 + (q.correctAnswer as number))}`);
            ...
            console.log(`   ✓ Answer: ${q.correctAnswer === 0 ? 'A' : 'B'} (${q.correctAnswer === 0 ? q.option1 : q.option2})`);
          } else if (q.type === 'numerical') {
            console.log(`   ✓ Answer: ${q.correctAnswer}${q.unit || ''}`);
          }
        });
        console.log('\n' + '─'.repeat(60) + '\n');
      }
  ...
  console.log('\n┌─────────────────┬────────┬──────────┬──────────┬────────────┐');
  console.log('│ Question Type   │ Count  │ Actual % │ Target % │ Deviation  │');
  console.log('├─────────────────┼────────┼──────────┼──────────┼────────────┤');
  ...
  Object.keys(distribution).forEach(type => {
    ...
    const prefix = isSignificant ? '│ ⚠️  ' : '│ ';
    
    console.log(`${prefix}${type.padEnd(14)} │${count} │${actual} │${target} │${deviationPadded} │`);
  });
  ...
  if (mcPercentage < 30) {
    console.log('❌ Multiple-choice questions are SIGNIFICANTLY underrepresented!');
    console.log(`   Expected: 40%, Got: ${mcPercentage}%`);
    hasIssues = true;
  } else if (mcPercentage > 50) {
    console.log('❌ Multiple-choice questions are DOMINATING!');
    console.log(`   Expected: 40%, Got: ${mcPercentage}%`);
    hasIssues = true;
  } else if (mcPercentage < 35 || mcPercentage > 45) {
    console.log('⚠️  Multiple-choice questions deviate from target.');
    console.log(`   Expected: 40%, Got: ${mcPercentage}%`);
  } else {
    console.log('✅ Multiple-choice distribution looks good!');
    console.log(`   Expected: 40%, Got: ${mcPercentage}%`);
  }

  // Check True/False (target: 25%, range: 15-35%)
  if (tfPercentage < 15) {
    console.log('\n❌ True-false questions are SIGNIFICANTLY underrepresented!');
    console.log(`   Expected: 25%, Got: ${tfPercentage}%`);
    hasIssues = true;
  } else if (tfPercentage > 35) {
    console.log('\n❌ True-false questions are OVER-REPRESENTED!');
    console.log(`   Expected: 25%, Got: ${tfPercentage}%`);
    hasIssues = true;
  } else if (tfPercentage < 20 || tfPercentage > 30) {
    console.log('\n⚠️  True-false questions deviate from target.');
    console.log(`   Expected: 25%, Got: ${tfPercentage}%`);
  } else {
    console.log('\n✅ True-false distribution looks good!');
    console.log(`   Expected: 25%, Got: ${tfPercentage}%`);
  }

  // Check More-or-Less (target: 30%, range: 20-40%)
  if (molPercentage < 20) {
    console.log('\n❌ More-or-less questions are SIGNIFICANTLY underrepresented!');
    ...
    console.log('\n❌ More-or-less questions are OVER-REPRESENTED!');
    console.log(`   Expected: 30%, Got: ${molPercentage}%`);
    hasIssues = true;
  } else if (molPercentage < 25 || molPercentage > 35) {
    console.log('\n⚠️  More-or-less questions deviate from target.');
    console.log(`   Expected: 30%, Got: ${molPercentage}%`);
  } else {
    console.log('\n✅ More-or-less distribution looks good!');
    console.log(`   Expected: 30%, Got: ${molPercentage}%`);
  }

  // Check Numerical (target: 5%, range: 0-15%)
  if (numPercentage > 15) {
    console.log('\n❌ Numerical questions are OVER-REPRESENTED!');
    console.log(`   Expected: 5%, Got: ${numPercentage}%`);
    hasIssues = true;
  } else if (numPercentage > 10) {
    console.log('\n⚠️  Numerical questions are somewhat over-represented.');
    console.log(`   Expected: 5%, Got: ${numPercentage}%`);
  } else {
    console.log('\n✅ Numerical distribution looks good!');
    console.log(`   Expected: 5%, Got: ${numPercentage}%`);
  }

  if (!hasIssues) {
    console.log('\n🎉 Overall distribution is within acceptable ranges!');
  }
...
async function main() {
  ...
  console.log(`  Gemini: ${hasGeminiKey ? '✓ Set' : '✗ Not set'}`);
  console.log(`  OpenAI: ${hasOpenAIKey ? '✓ Set' : '✗ Not set'}`);
